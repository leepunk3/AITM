...
export default function AdminTrademarkReviewPage() {
  ...
  const overallRiskLabel = useMemo(() => {
    ...
    return risk ? riskLabelMap[risk] : "미분석";
  }, [result]);
...
function DashboardView({ result, form }: { result: ReviewResult | null; form: ReviewInput }) {
  ...
  const summary = result.summary ?? {
    overallRisk: "REVIEW_NEEDED",
    finalOpinion: "결과 없음",
    keyIssues: [],
    recommendedAction: "추가 검토 필요",
  };

  const markAnalysis = result.markAnalysis ?? {
    originalMark: form.markText,
    normalizedMark: form.markText,
    detectedLanguage: [],
    structure: "",
    semanticNotes: [],
  };

  const goodsAnalysis = result.goodsAnalysis ?? {
    inputGoodsServices: form.goodsServices,
    categoryGuess: [],
    descriptiveElements: [],
  };

  const grounds = Array.isArray(result.grounds) ? result.grounds : [];
  ...
  return (
    <div className="list-col">
      <div className="report-box">
        ...
        <p>
          <strong>종합 의견:</strong> {summary.finalOpinion || "-"}
        </p>
        <p>
          <strong>권장 조치:</strong> {summary.recommendedAction || "-"}
        </p>
      ...
      <div className="report-box">
        <h3>표장 및 물품 분석</h3>
        <p>
          <strong>정규화 표장:</strong> {markAnalysis.normalizedMark || "-"}
        </p>
        <p>
          <strong>언어:</strong> {(markAnalysis.detectedLanguage || []).join(", ") || "-"}
        </p>
        <p>
          <strong>구조:</strong> {markAnalysis.structure || "-"}
        </p>
        <p>
          <strong>의미 메모:</strong> {(markAnalysis.semanticNotes || []).join(" / ") || "-"}
        </p>
        <p>
          <strong>카테고리 추정:</strong> {(goodsAnalysis.categoryGuess || []).join(", ") || "-"}
        </p>
        <p>
          <strong>성질요소:</strong> {(goodsAnalysis.descriptiveElements || []).join(", ") || "-"}
        </p>
      </div>
      ...
      <div className="list-col">
        {grounds.length === 0 ? (
          <div className="ground-card">
            <p className="ground-text">조문별 검토 결과가 없습니다.</p>
          </div>
        ) : (
          grounds.map((ground, index) => {
            const safeRisk = ground?.risk || "REVIEW_NEEDED";
            return (
              <div className="ground-card" key={`${ground?.article || "검토항목"}-${index}`}>
                <div className="ground-top">
                  <div className="ground-title">
                    {formatGroundTitle(ground?.article || "검토항목", ground?.title || "분석")}
                  </div>
                  <span className={`badge ${riskBadgeClass(safeRisk)}`}>{riskLabelMap[safeRisk]}</span>
                </div>
...
function ExpertReportView({ result, expertReport }: { result: ReviewResult | null; expertReport: string }) {
  ...
  const reportBody = result.reportBody ?? {
    executiveSummary: "",
    legalAssessment: "",
    practicalComment: "",
    filingStrategy: "",
  ...
  return (
    <div className="report-box">
      ...
      <p>{reportBody.executiveSummary || "-"}</p>

      <h4>2. Legal Assessment</h4>
      <p>{reportBody.legalAssessment || "-"}</p>

      <h4>3. Practical Comment</h4>
      <p>{reportBody.practicalComment || "-"}</p>

      <h4>4. Filing Strategy</h4>
      <p>{reportBody.filingStrategy || "-"}</p>
    ...
    <div className="report-box">
      ...
      {loading ? (
        ...
        <table className="history-table">
          ...
          <tbody>
            {history.map((row) => {
              const risk = row?.overallRisk || "REVIEW_NEEDED";
              return (
                <tr key={row.id}>
                  <td>{row.createdAt}</td>
                  <td>{row.reviewer}</td>
